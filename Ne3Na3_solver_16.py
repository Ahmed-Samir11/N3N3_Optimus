...
def solver(env: LogisticsEnvironment, alpha: float = 0.3, use_2opt: bool = True) -> Dict:
    """Main entry: Optimized Hungarian + Clarke–Wright + 2-opt, with fallbacks.
    
    Args:
        env: LogisticsEnvironment instance
        alpha: Balance between cost (1.0) and distance (0.0). Default 0.3 prioritizes distance.
    ...
    # Try to merge routes with Clarke–Wright
...
def my_solver(env: LogisticsEnvironment) -> Dict:
    """Wrapper with optimized defaults: prioritize distance over cost."""
    return solver(env, alpha=0.3, use_2opt=True)
