"""
Ne3Na3 Solver 51 - EXTREME Cost Minimization via Consolidation
==================================================================

TOP PRIORITY: MINIMIZE COST - Target $1,500 average (vs current $3,341)

RADICAL STRATEGY:
1. USE ONLY HEAVYTRUCKS - Best cost per capacity ($0.17 vs $0.27 LightVan)
2. MAXIMUM CONSOLIDATION - Pack 99% capacity, use MINIMUM vehicles
3. SINGLE WAREHOUSE TRIPS - No multi-warehouse routes
4. DISTANCE IS SECONDARY - Fixed cost dominates (90% of total)

KEY INSIGHT from analysis:
- HeavyTruck: $1,200 fixed, can hold 5000kg / 20m³
- Total demand: 10,980kg / 43.92m³
- Theoretical min: 2.2 HeavyTrucks = $2,635 (vs current $3,341)
- GOAL: Use exactly 2-3 HeavyTrucks only

Cost Breakdown Target:
- 2 HeavyTrucks: $2,400 fixed + ~$300 variable = $2,700 total
- 3 HeavyTrucks: $3,600 fixed (only if 2 can't fit all orders)
"""
...
class VehicleSelectorML:
    ...
    def rank_vehicles(self, env, vehicles, orders, warehouse_node):
        """
        EXTREME COST OPTIMIZATION: ONLY use HeavyTrucks!
        
        HeavyTruck has best cost-per-capacity ratio ($0.17 vs LightVan $0.27)
        Strategy: Fill HeavyTrucks to 98%+ capacity, minimize vehicle count
        """
        try:
            # FILTER: Only HeavyTrucks
            heavy_trucks = [v for v in vehicles if 'HeavyTruck' in v.id or 'Heavy' in v.type]
            
            if not heavy_trucks:
                # Fallback: if no HeavyTrucks, use what's available
                print("[WARNING] No HeavyTrucks available, using fallback vehicles")
                return [(1.0, v) for v in vehicles]
            
            # Calculate demand
            features = self.extract_order_batch_features(env, orders, warehouse_node)
            total_weight = features[0] * 100.0
            total_volume = features[1] * 5.0
            
            vehicle_scores = []
            
            for vehicle in heavy_trucks:
                capacity_weight = getattr(vehicle, 'capacity_weight', 5000.0)
                capacity_volume = getattr(vehicle, 'capacity_volume', 20.0)
                
                # Check if orders fit
                if total_weight > capacity_weight or total_volume > capacity_volume:
                    # Can't fit - very low score
                    fitness = 0.01
                else:
                    # Base fitness: prefer vehicles that will be HIGHLY utilized
                    utilization = max(total_weight / capacity_weight, total_volume / capacity_volume)
                    
                    # MAXIMIZE utilization (want 95-99%)
                    if utilization >= 0.95:
                        fitness = 100.0  # EXCELLENT - nearly full
                    elif utilization >= 0.85:
                        fitness = 50.0   # GOOD
                    elif utilization >= 0.70:
                        fitness = 20.0   # OK
                    else:
                        fitness = 1.0    # POOR - wasting capacity
            ...
            # Emergency fallback
            heavy = [v for v in vehicles if 'Heavy' in str(v.id) or 'Heavy' in str(v.type)]
            if heavy:
                return [(1.0, v) for v in heavy]
            return [(1.0, v) for v in vehicles]
...
def solver(env: Any) -> Dict:
    """
    ULTRA COST-OPTIMIZED SOLVER
    Priority: MINIMIZE cost via vehicle consolidation
    Strategy: Use ONLY HeavyTrucks (best cost/capacity), pack to 99%
    Target: $1,500 average (vs $3,341 baseline)
    """
    print("\n[SOLVER 51] EXTREME Cost Optimization - HeavyTruck-Only Strategy")
    print("=" * 80)
    
    # Debug: Show vehicle costs
    vehicles = list(env.get_all_vehicles())
    heavy_trucks = [v for v in vehicles if 'Heavy' in v.type]
    print(f"Available HeavyTrucks: {len(heavy_trucks)}")
    if heavy_trucks:
        print(f"HeavyTruck specs: ${heavy_trucks[0].fixed_cost} fixed, ${heavy_trucks[0].cost_per_km}/km")
        print(f"Capacity: {heavy_trucks[0].capacity_weight}kg / {heavy_trucks[0].capacity_volume}m³")
    ...
    print(f"Total demand: {total_weight:.0f}kg / {total_volume:.1f}m³")
    
    if heavy_trucks:
        min_trucks_weight = total_weight / heavy_trucks[0].capacity_weight
        min_trucks_volume = total_volume / heavy_trucks[0].capacity_volume
        min_trucks = max(min_trucks_weight, min_trucks_volume)
        print(f"Theoretical min HeavyTrucks needed: {min_trucks:.1f}")
        print(f"Theoretical min cost: ${min_trucks * heavy_trucks[0].fixed_cost:.0f} (fixed only)")
    
    print("=" * 80)
    
    result = ml_vehicle_optimizer(env, time_budget=90.0)
    
    # Debug: Show actual cost
    try:
        cost = env.calculate_solution_cost(result)
        print(f"\n[SOLVER 51] Final solution cost: ${cost:,.0f}")
        print(f"Vehicles used: {len(result.get('routes', []))}")
        
        vehicle_types = {}
        for route in result.get('routes', []):
            v = env.get_vehicle_by_id(route['vehicle_id'])
