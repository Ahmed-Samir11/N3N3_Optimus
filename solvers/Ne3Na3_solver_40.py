"""
Ne3Na3 Solver 40 - ML-Driven Cost Optimization
==================================================================
...
def ultra_cost_optimizer(env: Any, time_budget: float = 600.0) -> Dict:
    """
    ML-driven cost optimizer with FAST PATH for simple problems.
    
    Phase 0 (4s): Train COST predictor and clusterer on initial solutions
    Phase 1 (8s): Generate ML-cost-optimized solutions
    Phase 2 (8s): Fast memetic algorithm with cost focus (REDUCED!)
    Phase 3 (30s): 2-opt on best solutions
    Phase 4 (5s): Final refinement
    """
    start_time = time.time()
    
    # FAST PATH: For small problems, skip ML and return quickly
    total_orders = len(env.get_all_order_ids())
    if total_orders <= 20:  # Small problem - use fast greedy approach
        shipments_by_wh = smart_allocate_orders(env)
        solution = build_initial_solution(env, shipments_by_wh)
    ...
    # PHASE 2: Fast memetic evolution (REDUCED to 8s)
    phase2_end_time = start_time + 20.0  # 12s + 8s
    
    iterations = 0
    no_improvement_count = 0
    max_no_improvement = 20
    
    while time.time() < phase2_end_time and time.time() - start_time < time_budget - 10:  # 10s safety buffer!
        iterations += 1
    ...
    # PHASE 3: 2-opt optimization (20 seconds max, with 10s safety buffer)
    phase3_end_time = min(start_time + 35.0, start_time + time_budget - 10)  # Leave 10s buffer!
    
    # Apply 2-opt to BEST solution
    ...
    for route in best_solution.get("routes", []):
        ...
        # Reduced time per route
        time_per_route = min(8.0, remaining_time / (len(best_solution.get("routes", [])) + 1))
        
        # 2-opt with timeout protection
    ...
    # SKIP PHASE 4 - Return immediately to avoid timeout risk!
...
def solver(env: Any) -> Dict:
    """
    """
    return ultra_cost_optimizer(env, time_budget=45.0)