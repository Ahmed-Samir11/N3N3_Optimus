"""
Ne3Na3 Solver 49 - ROBUST ML-Driven Vehicle Selection with Error Handling
==================================================================

FIXES from Solver 48:
1. Added error handling to prevent empty solutions (Scenario 3 crash fix)
2. Improved time budget allocation for better performance
3. Added fallback greedy solution if ML optimization fails
4. Better validation that never returns empty routes
5. Conservative 2-opt timing to avoid timeouts

Target: 100% fulfillment across ALL scenarios with competitive costs
"""
...
class VehicleSelectorML:
    ...
    def rank_vehicles(self, env, vehicles, orders, warehouse_node):
        ...
        """
        if not self.trained:
            # Fallback: capacity-based ranking
            return [(getattr(v, 'capacity_weight', 0) + getattr(v, 'capacity_volume', 0), v) 
                    for v in vehicles]
        ...
        try:
            ...
            for vehicle in vehicles:
                ...
                # Get predicted fitness from appropriate model
                if vehicle_type == "HeavyTruck" and self.heavy_truck_model.trained:
                    fitness = float(self.heavy_truck_model.predict([features])[0])
                elif vehicle_type == "MediumTruck" and self.medium_truck_model.trained:
                    fitness = float(self.medium_truck_model.predict([features])[0])
                elif vehicle_type == "LightVan" and self.light_van_model.trained:
                    fitness = float(self.light_van_model.predict([features])[0])
                else:
                    # No model for this type - use capacity fallback
                    fitness = getattr(vehicle, 'capacity_weight', 0) * 0.1
                ...
                if total_weight > capacity_weight or total_volume > capacity_volume:
                    # Penalize vehicles that can't fit the load
                    fitness *= 0.1
                else:
                    # Bonus for good utilization (70-95% is ideal)
                    utilization = max(total_weight / capacity_weight, total_volume / capacity_volume)
                    if 0.7 <= utilization <= 0.95:
                        fitness *= 1.3  # Bonus for good fit
                    elif utilization < 0.5:
                        fitness *= 0.8  # Penalty for underutilization
                
                vehicle_scores.append((fitness, vehicle))
            
            # Sort by fitness (descending) - add index to break ties
            vehicle_scores_with_idx = [(score, idx, vehicle) for idx, (score, vehicle) in enumerate(vehicle_scores)]
            vehicle_scores_with_idx.sort(reverse=True, key=lambda x: x[0])
            return [(score, vehicle) for score, idx, vehicle in vehicle_scores_with_idx]
        
        except Exception as e:
            print(f"[VehicleML] Ranking failed: {e}")
            # Fallback
            return [(getattr(v, 'capacity_weight', 0), v) for v in vehicles]
...
def validate_and_fix_solution(env: Any, solution: Dict) -> Dict:
    ...
    CRITICAL: NEVER return empty solution - use fallback if all routes invalid.
    """
    valid_routes = []
    removed_routes = 0
    ...
    # CRITICAL FIX: If all routes were removed, generate fallback greedy solution
    if len(valid_routes) == 0:
        print(f"[VALIDATION] WARNING: All routes invalid! Generating fallback solution...")
        try:
            shipments_by_wh = smart_allocate_orders(env)
            fallback_solution = build_ml_vehicle_optimized_solution(env, shipments_by_wh, VehicleSelectorML())
            return fallback_solution
        except Exception as e:
            print(f"[VALIDATION] Fallback failed: {e}. Returning original solution.")
            return solution
...
def ml_vehicle_optimizer(env: Any, time_budget: float = 45.0) -> Dict:
    ...
    IMPROVED TIME ALLOCATION (more conservative):
    Phase 0 (3s): Train vehicle selector from initial solutions
    Phase 1 (8s): Generate ML-optimized solutions with smart vehicle choice
    Phase 2 (12s): Memetic evolution
    Phase 3 (15s): 2-opt optimization (conservative timing)
    Reserve 5s buffer to avoid timeout
    """
    start_time = time.time()
    max_runtime = min(time_budget - 5.0, 115.0)  # Reserve 5s buffer, cap at 115s
    ...
    if total_orders <= 20:
        try:
            ...
            print(f"   [FAST PATH FAILED] {e}, using greedy fallback")
            return build_greedy_fallback(env)
    ...
    # PHASE 0: Train vehicle selector (3 seconds)
    training_solutions = []
    phase0_end_time = start_time + 3.0
    ...
    while time.time() < phase0_end_time and len(training_solutions) < 3:
        try:
            ...
            print(f"[Phase 0] Training solution generation failed: {e}")
            break
    
    # Ensure we have at least one solution
    if len(training_solutions) == 0:
        print("[Phase 0] No training solutions, using fallback")
        return build_greedy_fallback(env)
    ...
    try:
        ...
        print(f"[Phase 0] Training failed: {e}, using untrained selector")
    
    # PHASE 1: Generate ML-optimized population (8 seconds)
    phase1_end_time = start_time + 11.0
    population = training_solutions.copy()
    
    while time.time() < phase1_end_time and time.time() - start_time < max_runtime:
        try:
            # Use TRAINED vehicle selector
            sol = build_ml_vehicle_optimized_solution(env, shipments_by_wh, vehicle_selector)
            population.append(sol)
            
            if len(population) >= 8:  # Reduced from 10 for faster iteration
                break
        except Exception as e:
            print(f"[Phase 1] Solution generation failed: {e}")
            break
    ...
    try:
        ...
        print(f"[Phase 1] Population scoring failed: {e}")
        population = training_solutions[:1]
    ...
    # PHASE 2: Memetic evolution (12 seconds)
    phase2_end_time = start_time + 23.0
    
    iterations = 0
    no_improvement_count = 0
    
    while time.time() < phase2_end_time and time.time() - start_time < max_runtime - 15:
        iterations += 1
        
        if no_improvement_count >= 15:  # Reduced from 20 for faster exit
            print(f"[Phase 2] Early stop after {iterations} iterations (no improvement)")
            break
        ...
        try:
            ...
            print(f"[Phase 2] Iteration {iterations} failed: {e}")
            break
    
    # PHASE 3: 2-opt optimization (15 seconds, conservative)
    phase3_end_time = min(start_time + max_runtime - 2, start_time + 38.0)
    
    best_routes_optimized = []
    for route in best_solution.get("routes", []):
        if time.time() >= phase3_end_time or time.time() - start_time >= max_runtime - 2:
            best_routes_optimized.append(route)
            continue
        
        remaining_time = phase3_end_time - time.time()
        if remaining_time <= 1:
            best_routes_optimized.append(route)
            continue
        
        # Conservative time allocation: max 4s per route
        time_per_route = min(4.0, remaining_time / (len(best_solution.get("routes", [])) + 1))
        
        try:
            improved_route = two_opt_route_optimization(env, route, max_time=time_per_route)
            best_routes_optimized.append(improved_route)
        except Exception as e:
            print(f"[Phase 3] Route optimization failed: {e}, using original")
            best_routes_optimized.append(route)
    ...
    try:
        ...
        print(f"[Phase 3] Evaluation failed: {e}, keeping pre-opt solution")
    
    elapsed = time.time() - start_time
    print(f"   Final: ${best_cost:.0f}, {best_fulfillment:.1f}%, Time: {elapsed:.1f}s")
    
    # Final validation
    try:
        best_solution = validate_and_fix_solution(env, best_solution)
    except Exception as e:
        print(f"[VALIDATION] Failed: {e}, returning unvalidated solution")
...
def build_greedy_fallback(env: Any) -> Dict:
    """
    EMERGENCY FALLBACK: Simple greedy solution when ML optimization fails.
    Prioritizes FULFILLMENT over cost.
    """
    print("[FALLBACK] Building greedy solution...")
    solution = {"routes": []}
    ...
    try:
        ...
        # For each warehouse, assign all orders to vehicles
        for wh_id, orders_map in shipments_by_wh.items():
            if not orders_map:
                continue
            ...
            for vehicle in vehicles:
                ...
                route_orders = []
                remaining = dict(orders_map)
                
                for order_id, sku_map in list(orders_map.items()):
                    # Check capacity
                    can_fit = True
                    total_weight = 0
                    total_volume = 0
                    ...
                    if total_weight <= vehicle.capacity_weight and total_volume <= vehicle.capacity_volume:
                        route_orders.append(order_id)
                        del orders_map[order_id]
                ...
                if route_orders:
                    ...
                    for oid in route_orders:
                        order_node = env.get_order_location(oid)
                        sku_map = remaining[oid]
                        
                        deliveries = [{"order_id": oid, "sku_id": sk, "quantity": q} 
                                    for sk, q in sku_map.items()]
                        
                        steps.append({"node_id": order_node, "pickups": [], 
        ...
        print(f"[FALLBACK] Generated {len(solution['routes'])} routes")
        return solution
    except Exception as e:
        print(f"[FALLBACK] CRITICAL: Failed to build fallback solution: {e}")
        return {"routes": []}
...
def solver(env: Any) -> Dict:
    return ml_vehicle_optimizer(env, time_budget=45.0)
