"""
Ne3Na3 Solver 41 - ML-Driven DYNAMIC Vehicle Selection
==================================================================

INNOVATION: Intelligent vehicle selection using ML-trained scoring model

Key Features:
1. VehicleSelectorML: Learns optimal vehicle type from order characteristics
2. Multi-criteria scoring: weight, volume, distance, density → vehicle fitness
3. Dynamic adaptation: Adjusts strategy based on real-time packing success
4. Ensemble vehicle ranking: Combines multiple scoring methods
5. Cost-efficiency prediction: Balances fixed vs variable costs intelligently

Difference from 40: REPLACES hardcoded multipliers with ML-trained selector
Target: Lower costs through smarter vehicle-order matching
"""
...
def build_ml_vehicle_optimized_solution(env: Any, shipments_by_wh: Dict, vehicle_selector: VehicleSelectorML) -> Dict:
    ...
    for wh_id, wh_ship in shipments_by_wh.items():
        ...
        if vehicle_selector.trained and available_vehicles:
            ...
            sorted_local_vehicles = sorted(available_vehicles, 
                                          key=lambda v: (getattr(v, 'capacity_weight', 0), 
                                                        getattr(v, 'capacity_volume', 0)), 
                                          reverse=True)
        
        # Pack using ML-optimized vehicle order
        ...
        # SECONDARY: If orders still remain, try ANY unused vehicle
        if remaining:
            for vehicle in all_vehicles:
                if not remaining or vehicle.id in used_vehicles:
                    continue
...
def ml_vehicle_optimizer(env: Any, time_budget: float = 45.0) -> Dict:
    ...
    phase3_end_time = min(start_time + 43.0, start_time + time_budget - 2)
    
    best_routes_optimized = []
    for route in best_solution.get("routes", []):
        if time.time() >= phase3_end_time or time.time() - start_time >= time_budget - 10:
            best_routes_optimized.append(route)
            continue
        
        remaining_time = min(phase3_end_time - time.time(), time_budget - 10 - (time.time() - start_time))
        if remaining_time <= 1:
            best_routes_optimized.append(route)
            continue
        
        time_per_route = min(6.0, remaining_time / (len(best_solution.get("routes", [])) + 1))
...
def solver(env: Any) -> Dict:
    """Entry point - ML-driven dynamic vehicle selection optimizer."""
