"""
Ne3Na3 Solver 34 - Speed-Optimized with Zero-Distance Prevention
==================================================================

Optimizations from Solver 31:
1. Removed verbose debugging output (50%+ time savings on I/O)
2. Smarter time allocation (60s budget for concurrent scenarios)
3. Early stopping in optimization phases (convergence detection)
4. Reduced population size (8 vs 15 solutions)
5. Faster memetic iterations with better convergence

Key Features:
- Zero-Distance Prevention (4 layers of validation)
- Smart multi-warehouse order allocation
- Fast K-means clustering
- Efficient memetic algorithm
- Time-aware optimization phases

Target: <60 seconds per scenario for concurrent evaluation
"""
...
def ultra_cost_optimizer(env: Any, time_budget: float = 600.0) -> Dict:
    ...
    Reduced from 10min to 60s budget with early stopping.
    
    Phase 1 (10s): Generate initial solutions
    Phase 2 (25s): Fast memetic algorithm with convergence detection
    Phase 3 (15s): 2-opt on best solutions
    Phase 4 (10s): Final refinement
    """
    start_time = time.time()
    ...
    # PHASE 1: Generate diverse initial population (10 seconds)
    phase1_end_time = start_time + min(10.0, time_budget * 0.17)
    population = []
    ...
    while time.time() < phase1_end_time and time.time() - start_time < time_budget:
        ...
        if len(population) >= 8:  # Reduced from 20
            break
    
    # Evaluate and sort
    population_scored = [(solution_cost(env, sol), sol) for sol in population]
    population_scored.sort()
    population = [sol for _, sol in population_scored[:8]]  # Reduced from 15
    
    best_solution = population[0]
    best_cost = solution_cost(env, best_solution)
    best_fulfillment = evaluate_fulfillment(env, best_solution)
    
    # PHASE 2: Fast memetic evolution with early stopping (25 seconds)
    phase2_end_time = start_time + 35.0  # 10s + 25s
