"""
Ne3Na3 Solver 57: Hybrid Memetic + NetworkX Optimizer
======================================================
        ...
        for truck in available_vehicles:
            ...
            if packed:
                ...
                for node in delivery_nodes:
                    ...
                    # Add path nodes
                    if steps:
                        last_node = steps[-1]["node_id"]
                        path, _ = dijkstra_shortest_path(env, last_node, node)
                        if path:
                ...
                if steps:
                    last_node = steps[-1]["node_id"]
                    path, _ = dijkstra_shortest_path(env, last_node, wh_node)
                    if path:
                        for intermediate in path[1:-1]:
                            steps.append({"node_id": intermediate, "pickups": [], "deliveries": [], "unloads": []})
