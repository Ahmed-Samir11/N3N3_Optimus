...
def find_n_best_insertions(env: Any, order_id: str, solution: Dict, dist_matrix: Dict, n: int = 2) -> List[Tuple[float, str, int, int]]:
    ...
    for route_idx, route in enumerate(routes):
        ...
        vehicle = env.get_vehicle(vehicle_id)
        ...
        for step in steps:
            for delivery in step.get('deliveries', []):
                del_order_id = delivery['order_id']
                del_reqs = env.get_order_requirements(del_order_id)
                current_weight += sum(env.skus[sku].weight * qty for sku, qty in del_reqs.items())
                current_volume += sum(env.skus[sku].volume * qty for sku, qty in del_reqs.items())
    ...
    # Find closest warehouse that has inventory
    best_warehouse = None
    min_dist = float('inf')
    ...
    if best_warehouse:
        # Find an available vehicle from this warehouse
        for vehicle in env.get_all_vehicles():
            if vehicle.capacity_weight >= order_weight and vehicle.capacity_volume >= order_volume:
                # Check if vehicle is already used
                used_vehicles = {r['vehicle_id'] for r in routes}
                if vehicle.id not in used_vehicles:
                    # Cost of new route: fixed cost + distance to order + distance back
                    wh_node = best_warehouse.location.id
                    ...
                    insertions.append((new_route_cost, vehicle.id, -1, -1))  # -1 indicates new route
                    break
...
def calculate_insertion_cost(env: Any, route: Dict, order_node: int, position: int, 
    ...
    try:
        ...
        # Get warehouse node (first pickup)
        wh_node = None
        for step in steps:
            if step.get('pickups'):
        ...
        # Current route: home -> warehouse -> deliveries -> home
        prev = home_node
        current_cost += dist_matrix.get((prev, wh_node), 0)
        prev = wh_node
        
        for node in node_sequence:
            current_cost += dist_matrix.get((prev, node), 0)
            prev = node
        current_cost += dist_matrix.get((prev, home_node), 0)
        
        # New route with insertion
        new_sequence = node_sequence[:position] + [order_node] + node_sequence[position:]
        
        new_cost = 0.0
        prev = home_node
        new_cost += dist_matrix.get((prev, wh_node), 0)
        prev = wh_node
        
        for node in new_sequence:
            new_cost += dist_matrix.get((prev, node), 0)
            prev = node
        new_cost += dist_matrix.get((prev, home_node), 0)
        
        return (new_cost - current_cost) * vehicle.cost_per_km
...
def insert_order_into_route(env: Any, solution: Dict, order_id: str, vehicle_id: str, 
    ...
    if route_idx == -1:
        ...
        # Build route: home -> warehouse (pickup) -> order (delivery) -> home
        new_route = {
            ...
            "steps": [
                ...
                {
                    "node_id": wh_node,
                    "pickups": [{"warehouse_id": best_wh.id, "sku_id": sku, "quantity": qty} 
        ...
        if position >= len(delivery_indices):
            # Insert at end (before return to depot)
            insert_idx = len(steps) - 1
        elif position == 0:
            # Insert at beginning (after pickup)
            insert_idx = next(i for i, s in enumerate(steps) if s.get('deliveries'))
        else:
            insert_idx = delivery_indices[position - 1] + 1
        
        # Create new delivery step
        new_step = {
            "node_id": order_node,
            "pickups": [],
            "deliveries": [{"order_id": order_id, "sku_id": sku, "quantity": qty} 
        ...
        # Update pickups to include new order requirements
        for step in steps:
            if step.get('pickups'):
                # Merge with existing pickups
                for sku, qty in order_reqs.items():
                    found = False
                    for pickup in step['pickups']:
                        if pickup['sku_id'] == sku:
                            pickup['quantity'] += qty
                            found = True
                            break
                    if not found:
                        step['pickups'].append({
                            "warehouse_id": step['pickups'][0]['warehouse_id'] if step['pickups'] else None,
                            "sku_id": sku,
                            "quantity": qty
                        })
                break
...
def calculate_solution_cost(env: Any, solution: Dict, dist_matrix: Dict) -> float:
    ...
    for route in solution.get("routes", []):
        vehicle = env.get_vehicle(route['vehicle_id'])
        route_dist = 0.0
