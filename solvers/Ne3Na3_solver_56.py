"""
Ne3Na3 Solver 54: Hybrid Memetic + NetworkX Optimizer
======================================================
...
def mutate_solution(env: Any, solution: Dict, shipments_by_wh: Dict, remove_frac: float = 0.05) -> Dict:
    ...
    Fixed 5% removal rate for stability.
    """
    all_orders = []
    for r in solution.get("routes", []):
        for s in r.get("steps", []):
            for d in s.get("deliveries", []):
                all_orders.append(d.get("order_id"))
    
    if not all_orders:
        return solution
    
    uniq = list(set(all_orders))
    num_remove = max(1, int(len(uniq) * remove_frac))
    to_remove = set(random.sample(uniq, min(num_remove, len(uniq))))
    ...
    # Try to reinsert removed orders
...
def solver(env: Any) -> Dict:
    """
    Hybrid solver combining Solver 53 and Solver 13.
    """
    clear_caches()
    
    print("\n[SOLVER 54] Hybrid Memetic + NetworkX Optimizer")
    print("=" * 80)
    ...
    # PHASE 1: Simplified evolution (12 seconds - Solver 13 pattern)
    solution = memetic_solver(env, time_budget=12.0)
