"""
Ne3Na3 Solver 21 - Nearest Neighbor Route Optimization
========================================================

Improvements over Solver 15:
best params searching.
"""
...
def adaptive_memetic_solver(
    ...
    """
    Improved memetic algorithm with tunable hyperparameters.
    ...
    for i in range(pop_size):
        ...
        if i > 0 and sol.get("routes") and random.random() < perturbation_prob:
            # Small perturbation
            if len(sol["routes"]) > 1:
    ...
    while time.time() - start_time < time_budget and iterations < max_iterations:
        ...
        parent1 = max(random.sample(population, min(tournament_size, len(population))), key=lambda s: evaluate_fulfillment(env, s))
        parent2 = max(random.sample(population, min(tournament_size, len(population))), key=lambda s: evaluate_fulfillment(env, s))
...
def solver(env: Any) -> Dict:
    # Best parameters found by hyperparameter tuning (tune_solver_params.py)
    # Achieved: 100% fulfillment, Cost: $6,930.37
    best_params = {
        "time_budget": 15.0,
        "pop_size": 15,
        "max_iterations": 100,
