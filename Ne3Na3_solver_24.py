...
def solver(env: LogisticsEnvironment, alpha: float = 0.7, use_2opt: bool = True) -> Dict:
    """Main entry: Optimized Hungarian + Clarke–Wright + 2-opt, with fallbacks.
    
    Args:
        env: LogisticsEnvironment instance
        alpha: Balance between cost (1.0) and distance (0.0). Default 0.7 prioritizes cost.
        use_2opt: Whether to apply 2-opt optimization to multi-order routes.
    ...
    used_vehicles: set = set()  # Track vehicles already assigned to routes

    # Try Hungarian with cost/distance balance
    assignments = opt.assign_orders_hungarian(alpha=alpha)
    if not assignments:
        # Fallback to greedy
        assignments = opt.assign_orders()

    # Pre-consolidation: Group orders by (vehicle, warehouse)
    # Hungarian assigns 1 order per vehicle, but we want to consolidate later
    from collections import defaultdict
    pre_groups: dict = defaultdict(list)
    for (o, v, w) in assignments:
        pre_groups[(v, w)].append(o)
    
    # Use pre-consolidated groups directly (no Clarke-Wright expansion)
    groups = [(v, w, orders) for (v, w), orders in pre_groups.items()]

    # Build routes and track used vehicles
    for (vehicle_id, warehouse_id, order_ids) in groups:
        # Skip if vehicle already used (extra safety check)
