...
def solver(env: LogisticsEnvironment, alpha: float = 0.7, use_2opt: bool = True) -> Dict:
    ...
    # Use greedy consolidation instead of Hungarian (allows multiple orders per vehicle)
    assignments = opt.assign_orders_greedy_consolidated(alpha=alpha)
    if not assignments:
        # Ultimate fallback
        print("Warning: No assignments from greedy consolidation")
        return sol

    # Group orders by (vehicle, warehouse) - this gives us one route per vehicle
    ...
    for (vehicle_id, warehouse_id, order_ids) in groups:
        # Skip if vehicle already used (safety check)
    ...
    # Fallback for orders left unassigned: allow multi-warehouse pickups
